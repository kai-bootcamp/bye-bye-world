...
contract SellKEEYToken {
    uint16 private constant _rate = 10000;
    ...
    function getAmountToPay(uint128 number) public pure returns (uint256) {
        return number * _rate;
    }

    function buyKEEYToken(uint128 number) public payable {
        IERC20 usdtToken = IERC20(USDT_ADDRESS);
        IERC20 keeyToken = IERC20(KEEY_ADDRESS);

        keeyToken.transferFrom(WALLET, msg.sender, number);
        usdtToken.transferFrom(msg.sender, WALLET, getAmountToPay(number));
    }
}
